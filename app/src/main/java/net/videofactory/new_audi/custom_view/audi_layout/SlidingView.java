...
public class SlidingView extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        // 핵심 구현 부분으로써
        // 차일드뷰들을 겹치지 않게 아래로 차례대로 나열해서 배치한다.
        // 아래로 차례대로 배치를 해놔야 스크롤을 통해 아래로 이동하는것이 가능해진다.
        for (int i = 0; i < getChildCount(); i++) {
            int childTop = getChildAt(i).getMeasuredHeight() * i;
            getChildAt(i).layout(l, childTop, r, childTop + getChildAt(i).getMeasuredHeight());
        }
    }
    ...
    public int getRealHeight(){
        return getMeasuredHeight() * (getChildCount()-1);
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if(mCurPage == 0 && getScrollY() - y < 0){
                    scrollTo(0, 0); // 첫페이지라면 더이상 위로 올라가지 않음
                }else if(mCurPage == getChildCount()-1 && getScrollY() - y > getRealHeight()){
                    scrollTo(0, getRealHeight()); // 마지막페이지라면 더이상 아래로 내려가지 않음
                }else{
            ...
            case MotionEvent.ACTION_UP:
                ...
                int v = (int) mVelocityTracker.getYVelocity(); // x 축 이동 속도를 구함

                int gap = getScrollY() - mCurPage * getHeight(); // 드래그 이동 거리 체크
                int nextPage = mCurPage;

                // 드래그 속도가 SNAP_VELOCITY 보다 높거니 화면 반이상 드래그 했으면
                // 화면전환 할것이라고 nextPage 변수를 통해 저장.
                if ((v > SNAP_VELOCITY || gap < -getHeight() / 2) && mCurPage > 0) {
                    nextPage--;
                } else if ((v < -SNAP_VELOCITY || gap > getHeight() / 2)
                ...
                if (mCurPage != nextPage) { // 화면 전환 스크롤 계산
                    ...
                    move = getChildAt(0).getHeight() * nextPage - getScrollY();
                ...
                mScroller.startScroll(0, getScrollY(), 0, move, Math.abs(move));
    ...
    public void computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            ...
            }else if(mScroller.getCurrY() == getMeasuredHeight()){
...
//
//                //첫페이지이거나 마지막페이지인 경우 반대쪽 스크롤은 무시
//                if(mCurPage == 0 && getScrollY() - y < 0){
//                    break;
//                }else if(mCurPage == getChildCount()-1 && getScrollY() - y > getRealHeight()){
//                    break;
