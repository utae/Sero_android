...
package net.videofactory.new_audi.custom_view.range_bar;
...
//        canvas.drawCircle(mX, mY, mCircleRadiusPx, mCirclePaint);
        float width = getResources().getDisplayMetrics().density * 7;
        float height = getResources().getDisplayMetrics().density * 32;
        float radius = getResources().getDisplayMetrics().density * 2;
        canvas.drawRect(mX-(width/2), mY+(height/2), mX+(width/2), mY-(height/2), mCirclePaint);
        //Draw pin if pressed
